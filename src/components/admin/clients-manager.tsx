...
const ClientsManager = () => {
  ...
  const [newClient, setNewClient] = useState({ name: "", domain: "" });
  ...
  const addClient = async () => {
    if (newClient.name && newClient.domain) {
      try {
        ...
        // Recargar la lista completa en lugar de confiar en la respuesta
        await loadClients();
        setNewClient({ name: "", domain: "" });
      } catch (error) {
      ...
      <Card className="bg-white/80 backdrop-blur-sm border border-white/50 shadow-lg">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="client-domain">Dominio</Label>
              <Input
                id="client-domain"
          ...
          <Button
            ...
            disabled={!newClient.name || !newClient.domain}
          >
                ...
                >
                  {editingClient === client.id ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <Label htmlFor={`edit-name-${client.id}`}>
                            Nombre
                          </Label>
                          <Input
                        ...
                        <div>
                          <Label htmlFor={`edit-domain-${client.id}`}>
                            Dominio
                          </Label>
                          <Input
                            id={`edit-domain-${client.id}`}
